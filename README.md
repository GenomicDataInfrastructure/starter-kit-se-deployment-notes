...
In order to connect to the Beacon Network, update the `deploy/conf.py` file and specifically the values under the `general`, `project`, `service info` sections, to reflect the information of the organisation, as well as the `Web server configuration` to add the certificates for the TLS connection. Notice that the `uri` should include a `/` in the end, otherwise the URLS from the Beacon Network will not be correct.
